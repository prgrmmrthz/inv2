...
@Component({
  selector: 'app-boxes-inventory',
  templateUrl: './boxes-inventory.component.html',
  styleUrls: ['./boxes-inventory.component.css']
})
export class BoxesInventoryComponent implements OnInit {
  subs: any;
  productData: any = [];
  loading: boolean;
  invId: any;
  ...
  getData() {
    let params: Dsmodel = {
      ...
      table: 'products i',
      join: 'left join boxes s on s.product=i.id',
    ...
    this.be.callSP(a).subscribe(r => {
      ...
      if (x > 0) {
        ...
        dataToSave.forEach((v) => {
          ...
          let a = { fn: `boxesbeginningInventory(${v.id}, '${eval(v.qty).toFixed(2)}', ${this.invId})` };
          this.be.callSP(a).subscribe(r => {
            const x = r.res[0][0].result;
            if (x > 0) {
